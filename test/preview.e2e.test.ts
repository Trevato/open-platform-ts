// Per-change preview environments + copy-on-write data branches, end to end
// against real containers: a work item filed with a human-pushed branch is born
// `reviewing`, gets its own live URL whose data is a CoW clone of prod,
// isolated from prod, and torn down on merge. Docker-gated; off the M1 loop.
...
describe.skipIf(!sock)(
  "per-change preview environments + data branches",
  () => {
    ...
    test("a work item's change gets a live preview whose data is a CoW clone of prod, torn down on merge", async () => {
      ...
      const git = async (...a: string[]) => {
        ...
        if ((await p.exited) !== 0)
          throw new Error(
            `git ${a[0]}: ${await new Response(p.stderr).text()}`,
          );
      };
      ...
      const wk = await fetch(`${api}/api/v1/repos/plat/${app}/work`, {
        method: "POST",
        tls: { ca },
        headers: { authorization: admin, "content-type": "application/json" },
        body: JSON.stringify({ title: "preview", head: "feature" }),
      });
      expect(wk.status).toBe(201);
      const workItem = (await wk.json()) as { number: number; phase: string };
      // A human-pushed branch is born at `reviewing` with its change attached.
      expect(workItem.phase).toBe("reviewing");
      const prNum = workItem.number;
      ...
      // Merge → shipped; preview + its data branch are torn down.
      const merged = await fetch(
        `${api}/api/v1/repos/plat/${app}/work/${prNum}/merge`,
        {
          method: "POST",
          tls: { ca },
      ...
      expect(((await merged.json()) as { phase: string }).phase).toBe(
        "shipped",
      );
