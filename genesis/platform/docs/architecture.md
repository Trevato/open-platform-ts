...
(`packages/opd/src/platform.ts:137`) is the composition root: it opens the
...
die right there (`packages/opd/src/platform.ts:460`; see
...
first non-null response wins (`packages/opd/src/platform.ts:424-427`):
...
(`packages/opd/src/cli.ts:20`), laid out by `stateDir`
...
- **Desired state lives in git.** Every app spec is a file in the
  ...
  daemon on merge (`packages/opd/src/platform.ts:489`). See
...
- **One sovereign key.** Every secret is sealed to `key.age`, and every boot
  ...
  minutes later (`packages/opd/src/platform.ts:288`).
