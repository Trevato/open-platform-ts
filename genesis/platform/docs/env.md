...
| `CLAUDE_CODE_OAUTH_TOKEN`    | unset                           | The crew's inference credential, from `claude setup-token` (`packages/opd/src/platform.ts:318`). Absent, the crew idles. See [The crew](/docs/crew).                   |
...
| `OP_FORCE_FIRST_REVIEW_FAIL` | unset                           | Demo/test hook: fails the first crew review of every item so the rework loop can be watched (`packages/opd/src/platform.ts:499`). Leave unset in real use.             |
