...
| ---------------------------- | ------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `DOMAIN`                     | `plat.localtest.me`             | The platform's domain; every app gets a subdomain of it (`packages/opd/src/cli.ts:81`).                                                                                |
| `OP_ROOT`                    | `~/.op/<domain>`                | State root: store, repos, data dirs, certs (`packages/opd/src/cli.ts:15`).                                                                                             |
| `HTTP_PORT` / `HTTPS_PORT`   | `80` / `443`                    | The gate's listen ports. Defaults need elevated privileges; local dev usually sets both (`packages/opd/src/cli.ts:17-18`).                                             |
| `FORK_KEY_ACK`               | unset                           | `1` acknowledges sovereign-key custody for non-interactive boots; an interactive TTY also counts (`packages/opd/src/cli.ts:20`). See [Sovereignty](/docs/sovereignty). |
| `SEED`                       | unset                           | Seed tarball path for `op germinate`, as an alternative to the positional argument (`packages/opd/src/cli.ts:122`).                                                    |
| `OP_SRC`                     | unset                           | A managed clone of `plat/opd`; when set, `op up` runs supervised and a merge to `plat/opd` re-execs the daemon from its own source (`packages/opd/src/cli.ts:88-89`).  |
...
| `CLAUDE_CODE_OAUTH_TOKEN`    | unset                           | The crew's inference credential, from `claude setup-token` (`packages/opd/src/platform.ts:302`). Absent, the crew idles. See [The crew](/docs/crew).                   |
| `ANTHROPIC_API_KEY`          | unset                           | Composer fast lane only: a real API key skips the SDK subprocess and hits the Messages API directly (`packages/opd/src/crew/composer.ts:140`).                         |
| `OP_COMPOSER_MODEL`          | `claude-haiku-4-5`              | Model for the issue composer (`packages/opd/src/crew/composer.ts:33`).                                                                                                 |
...
| `OP_FORCE_FIRST_REVIEW_FAIL` | unset                           | Demo/test hook: fails the first crew review of every item so the rework loop can be watched (`packages/opd/src/platform.ts:482`). Leave unset in real use.             |
